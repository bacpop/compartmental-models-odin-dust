...
probs[] <- ((1 + exp(sigma_f))^pi_f_genotypes[i] * (1 + exp(sigma_w))^pi_w_genotypes[i]) * (1- (as.integer(time >= vacc_time) * vaccTypes[i] * v))
...
#y[] <- rpois(capacity/sum(Pop[1:species_no]) * (probs[i]/sum(probs[1:species_no])) * Pop[i] * (1-exp(m)) ) 
y[] <- rpois(capacity/sum(Pop[1:species_no]) * (probs[i]) * Pop[i] * (1-exp(m)) ) 
...
mig_num <- rpois(capacity * exp(m))
#Pop_mig[] <- rbinom(mig_num, migVec[i])
Pop_mig[] <- rpois(mig_num * migVec[i])
#Pop_mig[] <- mig_num * migVec[i]
