...
for(j in 1:length(Navajo_time_points_selected)){
  ...
  for (i in unique(Navajo_PP$Cluster)){
    for (k in unique(Navajo_PP$Serotype)) {
      Navajo_cluster_freqs_sero[[j]][which(unique(Navajo_PP$Cluster) == i),which(unique(Navajo_PP$Serotype) == k)] <- length(which(Navajo_meta_data[Navajo_meta_IDtoInd[Navajo_PP[Navajo_PP$Cluster == i,"Taxon"]],][Navajo_meta_data[Navajo_meta_IDtoInd[Navajo_PP[Navajo_PP$Cluster == i,"Taxon"]],]$Sero==k,]$Year==Navajo_time_points[j]))
    }
  }
}
for(j in 1:length(Navajo_time_points_selected)){
