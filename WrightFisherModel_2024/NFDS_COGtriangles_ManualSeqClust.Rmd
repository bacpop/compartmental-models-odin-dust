...
plot(0.11292912/(1+exp(4.95113406 *(1774*(0.62342733)-(1:1774)))), main = "Logistic Model Fit with ggCaller and PopPUNK", xlab = "Genes", ylab = "Selection Strength", col = col_clb[1], ylim = c(0, 0.12))
points(0.1144311/(1+exp((1774*(0.6243419)-(1:1774)))), col = col_clb[2])
points(0.03503935/(1+exp(1 *(1774*(0)-(1:1774)))), col = col_clb[3])
points(c(rep(0.0004164005 , 1774 * (1-0.3803173188)), rep(0.1180783495, 1774 * 0.3803173188)), col = col_clb[4])
points(rep(0.01931485 , 1774 * (1)), col = col_clb[5])
legend(0, 0.12, legend=c("Logistic, 5 params", "Logistic, 4 params", "Logistic, 3 params", "step function, 5 params", "step function, 3 params"),
...
LL_3param <- -335.9657
LL_5param <- -264.7959
lambda <- -2 * (LL_3param - LL_5param)
pchisq(lambda, 5-3, lower.tail = FALSE)

# significant!
# the 5-param model is significantly better than the 3-param model

# compare null model to 5-param model
LL_null <- -335.0503
LL_5param <- -264.7959
lambda <- -2 * (LL_null - LL_5param)
pchisq(lambda, 5-2, lower.tail = FALSE)

# compare null model to 3-param model
lambda <- -2 * (LL_null - LL_3param)
pchisq(lambda, 3-2, lower.tail = FALSE)

# compare 4-param logistic model to null model
LL_4param <- -262.2601
lambda <- -2 * (LL_null - LL_4param)
pchisq(lambda, 4-2, lower.tail = FALSE)

# compare 4-param logistic model to 3-param model
lambda <- -2 * (LL_3param - LL_4param)
