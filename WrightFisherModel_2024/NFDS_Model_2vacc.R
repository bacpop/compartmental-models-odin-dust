...
probs[] <- ((1 + exp(sigma_f))^pi_f_genotypes[i] * (1 + exp(sigma_w))^pi_w_genotypes[i]) * Pop[i] * (1- (as.integer(time >= vacc_time1) * as.integer(time < vacc_time2) * vaccTypes1[i] * v)) * (1- (as.integer(time >= vacc_time2) * vaccTypes2[i] * v))
...
vaccTypes1[] <- user() # Boolean vector (0/1) whether genotype is affected by vaccine1 (1) or not (0)
vaccTypes2[] <- user() # Boolean vector (0/1) whether genotype is affected by vaccine2 (1) or not (0)
v <- user() # effect size of vaccine on vaccine genotypes
vacc_time1 <- user() # time when the 1st vaccination happens / starts to have an effect
vacc_time2 <- user() # time when the 2nd vaccination happens / starts to have an effect
migVec[] <- user()
...
dim(vaccTypes1) <- species_no
dim(vaccTypes2) <- species_no
