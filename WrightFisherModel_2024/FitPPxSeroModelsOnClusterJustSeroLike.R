...
if(args[1] == "ggCaller" & args[2] == "PopPUNK"){
  ...
  peripost_mass_cluster_freq <- data.frame("year" = c(1, 2), rbind(PP_mass_sero_freq_2, PP_mass_sero_freq_3))
  names(peripost_mass_cluster_freq) <- c("year", as.character(1:sero_no))
...
index <- function(info) {
  list(run = c(sum_clust = info$index$Pop_sero),
       state = c(Pop_tot=info$index$Pop_tot, Pop = info$index$Pop))
}
