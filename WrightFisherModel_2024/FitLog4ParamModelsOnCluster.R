...
if(args[1] == "ggCaller" & args[2] == "PopPUNK"){
  ...
  output_filename <- "ggCaller_PopPUNK_log4"
  ...
  mass_clusters <- length(unique(seq_clusters$Cluster))
  avg_cluster_freq <- rep(1/mass_clusters, mass_clusters)
  output_filename <- "COGtriangles_PopPUNK_log4"
  ...
  avg_cluster_freq <- rep(1/mass_clusters, mass_clusters)
  output_filename <- "ggCaller_manSeqClusters_log4"
  ...
  mass_clusters <- length(unique(seq_clusters$SequenceCluster))
  avg_cluster_freq <- rep(1/mass_clusters, mass_clusters)
  output_filename <- "COGtriangles_manSeqClusters_log4"
}
...
complex_params <- c(Pop_ini, Pop_eq, Genotypes, capacity, delta, vaccTypes, species_no, gene_no, vacc_time, dt, migVec, K)
...
make_transform <- function(p) {
  function(theta){
    c(list(Pop_ini = p[1:mass_clusters],
           ...
           K = p[((((2 * mass_clusters + 1) + (gene_no * species_no) - 1) + 2 + gene_no) + no_clusters + 4 + species_no)]), as.list(theta))
  }
}
...
mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("L", 0.1432, min = 0, max = 1), mcstate::pmcmc_parameter("x0", 0.2, min = 0, max = 1), mcstate::pmcmc_parameter("m", 0.03, min = 0, max = 1), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 1)), proposal_matrix, make_transform(complex_params))
mcmc_pars$initial()
...
det_mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("L", parameter_mean_hpd[1], min = 0, max = 1), mcstate::pmcmc_parameter("x0", parameter_mean_hpd[2], min = 0, max = 1), mcstate::pmcmc_parameter("m", parameter_mean_hpd[3], min = 0, max = 1), mcstate::pmcmc_parameter("v", parameter_mean_hpd[4], min = 0, max = 1)), det_proposal_matrix, make_transform(complex_params))
