...
# make VT-vector with VT, NVT, and mixed
Nepal_VT_mixed <- rep(0, no_Nepal_PP)

for (i in unique(Nepal_PP$GPSC)){
  Nepal_VT_mixed[(which(unique(Nepal_PP$GPSC) == i))] <- (mean(as.integer(PCV10_VTs[Nepal_SeqToSero_dict[Nepal_PP[Nepal_PP$GPSC == i,"sample"]]])))
}
```
...
# Nepal_SeqYear_dict already exists
# Nepal_seq_clusters_dict also exists already

Nepal_GPSC_numbers_per_clust <- list()
for (i in Nepal_time_points) {
  print(length(which(Nepal_SeqYear_dict == i)))
  count_vec <- rep(0, length(unique(Nepal_seq_clusters_dict)))
  names(count_vec) <- unique(Nepal_seq_clusters_dict)
  counts_year <- table(Nepal_seq_clusters_dict[names(which(Nepal_SeqYear_dict == i))])
  count_vec[names(counts_year)] <- counts_year
  Nepal_GPSC_numbers_per_clust[[as.character(i)]] <- count_vec
}
...
seq_no_per_year_filtered_goodYears <- seq_no_per_year_filtered[which(seq_no_per_year_filtered>=40)]
# do a version with only those years with >=40 sequences
Nepal_GPSC_numbers_per_good_year <- list()
for (i in unique(Nepal_seq_clusters_dict)) {
 count_vec <- rep(0, length(sort(unique(Nepal_SeqYear_dict))))
 names(count_vec) <- as.character(sort(unique(Nepal_SeqYear_dict)))
 counts_year <- table(Nepal_SeqYear_dict[names(which(Nepal_seq_clusters_dict == i))])
 count_vec[names(counts_year)] <- counts_year
 Nepal_GPSC_numbers_per_good_year[[as.character(i)]] <- count_vec[names(seq_no_per_year_filtered_goodYears)]
}
...
Nepal_VT_cols <- rep("black",length(Nepal_VT_mixed))
#Nepal_VT_cols[which(Nepal_VT_mixed == 0)] <- "black"
Nepal_VT_cols[which(Nepal_VT == 1)] <- "#CC79A7"

Nepal_VT_cols_2 <- rep("#00000050",length(Nepal_VT_mixed))
#Nepal_VT_cols_2[which(Nepal_VT_mixed == 0)] <- "#00000050"
Nepal_VT_cols_2[which(Nepal_VT == 1)] <- "#CC79A750"
...
# only VT / NVT
prev_points <- rep(0, length(seq_no_per_year_filtered))
plot((names(Nepal_GPSC_numbers_per_year[["1"]])), Nepal_GPSC_numbers_per_year[[order(Nepal_VT_cols)[1]]]/seq_no_per_year_filtered, type = "l", ylim = c(0, 1), col = sort(Nepal_VT_cols)[1], xlab = "Time", xaxt = "n")
axis(1, at = as.integer(names(Nepal_GPSC_numbers_per_year[["1"]])))
for (i in 1:length(unique(Nepal_seq_clusters_dict))) {
#for (i in 1:1) {
  #lines((names(Nepal_GPSC_numbers_per_year_conf_low[["1"]])), Nepal_GPSC_numbers_per_year_conf_low[[unique(Nepal_seq_clusters_dict)[i]]], col = Nepal_VT_mixed_cols_2[i])
  #lines((names(Nepal_GPSC_numbers_per_year_conf_up[["1"]])), Nepal_GPSC_numbers_per_year_conf_up[[unique(Nepal_seq_clusters_dict)[i]]], col = Nepal_VT_mixed_cols_2[i])
  #polygon(c((names(Nepal_GPSC_numbers_per_year[["1"]])), (names(Nepal_GPSC_numbers_per_year[["1"]]))), c(Nepal_GPSC_numbers_per_year_conf_low[[unique(Nepal_seq_clusters_dict)[i]]], Nepal_GPSC_numbers_per_year_conf_up[[unique(Nepal_seq_clusters_dict)[i]]]), col = "grey", density = 10, angle = 45)
  lines((names(Nepal_GPSC_numbers_per_year[["1"]])), prev_points + Nepal_GPSC_numbers_per_year[[unique(Nepal_seq_clusters_dict)[order(Nepal_VT_cols)[i]]]]/seq_no_per_year_filtered, col = sort(Nepal_VT_cols)[i])
  points((names(Nepal_GPSC_numbers_per_year[["1"]])), prev_points + Nepal_GPSC_numbers_per_year[[unique(Nepal_seq_clusters_dict)[order(Nepal_VT_cols)[i]]]]/seq_no_per_year_filtered, col = sort(Nepal_VT_cols)[i])
  polygon(c((names(Nepal_GPSC_numbers_per_year[["1"]])), rev(names(Nepal_GPSC_numbers_per_year[["1"]]))), c(prev_points + Nepal_GPSC_numbers_per_year[[unique(Nepal_seq_clusters_dict)[order(Nepal_VT_cols)[i]]]]/seq_no_per_year_filtered, rev(prev_points)), col = (Nepal_VT_cols_2)[order(Nepal_VT_cols)[i]], border = NA)
  prev_points <- prev_points + Nepal_GPSC_numbers_per_year[[unique(Nepal_seq_clusters_dict)[order(Nepal_VT_cols)[i]]]]/seq_no_per_year_filtered
}
abline(v = 2015, col = "red")

# absolute
prev_points <- rep(0, length(seq_no_per_year_filtered))
plot((names(Nepal_GPSC_numbers_per_year[["1"]])), Nepal_GPSC_numbers_per_year[[order(Nepal_VT_cols)[1]]], type = "l", ylim = c(0, 900), col = sort(Nepal_VT_cols)[1], xlab = "Time", xaxt = "n")
axis(1, at = as.integer(names(Nepal_GPSC_numbers_per_year[["1"]])))
for (i in 1:length(unique(Nepal_seq_clusters_dict))) {
#for (i in 1:1) {
  #lines((names(Nepal_GPSC_numbers_per_year_conf_low[["1"]])), Nepal_GPSC_numbers_per_year_conf_low[[unique(Nepal_seq_clusters_dict)[i]]], col = Nepal_VT_mixed_cols_2[i])
  #lines((names(Nepal_GPSC_numbers_per_year_conf_up[["1"]])), Nepal_GPSC_numbers_per_year_conf_up[[unique(Nepal_seq_clusters_dict)[i]]], col = Nepal_VT_mixed_cols_2[i])
  #polygon(c((names(Nepal_GPSC_numbers_per_year[["1"]])), (names(Nepal_GPSC_numbers_per_year[["1"]]))), c(Nepal_GPSC_numbers_per_year_conf_low[[unique(Nepal_seq_clusters_dict)[i]]], Nepal_GPSC_numbers_per_year_conf_up[[unique(Nepal_seq_clusters_dict)[i]]]), col = "grey", density = 10, angle = 45)
  lines((names(Nepal_GPSC_numbers_per_year[["1"]])), prev_points + Nepal_GPSC_numbers_per_year[[unique(Nepal_seq_clusters_dict)[order(Nepal_VT_cols)[i]]]], col = sort(Nepal_VT_cols)[i])
  points((names(Nepal_GPSC_numbers_per_year[["1"]])), prev_points + Nepal_GPSC_numbers_per_year[[unique(Nepal_seq_clusters_dict)[order(Nepal_VT_cols)[i]]]], col = sort(Nepal_VT_cols)[i])
  polygon(c((names(Nepal_GPSC_numbers_per_year[["1"]])), rev(names(Nepal_GPSC_numbers_per_year[["1"]]))), c(prev_points + Nepal_GPSC_numbers_per_year[[unique(Nepal_seq_clusters_dict)[order(Nepal_VT_cols)[i]]]], rev(prev_points)), col = (Nepal_VT_cols_2)[order(Nepal_VT_cols)[i]], border = NA)
  prev_points <- prev_points + Nepal_GPSC_numbers_per_year[[unique(Nepal_seq_clusters_dict)[order(Nepal_VT_cols)[i]]]]
}
abline(v = 2015, col = "red")

# only plot those year that have >=40 sequences
prev_points <- rep(0, length(seq_no_per_year_filtered_goodYears))
plot((names(Nepal_GPSC_numbers_per_good_year[["1"]])), Nepal_GPSC_numbers_per_good_year[[order(Nepal_VT_cols)[1]]]/seq_no_per_year_filtered_goodYears, type = "l", ylim = c(0, 1), col = sort(Nepal_VT_cols)[1], xlab = "Time", xaxt = "n")
axis(1, at = as.integer(names(Nepal_GPSC_numbers_per_year[["1"]])))
for (i in 1:length(unique(Nepal_seq_clusters_dict))) {
#for (i in 1:1) {
  #lines((names(Nepal_GPSC_numbers_per_year_conf_low[["1"]])), Nepal_GPSC_numbers_per_year_conf_low[[unique(Nepal_seq_clusters_dict)[i]]], col = Nepal_VT_mixed_cols_2[i])
  #lines((names(Nepal_GPSC_numbers_per_year_conf_up[["1"]])), Nepal_GPSC_numbers_per_year_conf_up[[unique(Nepal_seq_clusters_dict)[i]]], col = Nepal_VT_mixed_cols_2[i])
  #polygon(c((names(Nepal_GPSC_numbers_per_year[["1"]])), (names(Nepal_GPSC_numbers_per_year[["1"]]))), c(Nepal_GPSC_numbers_per_year_conf_low[[unique(Nepal_seq_clusters_dict)[i]]], Nepal_GPSC_numbers_per_year_conf_up[[unique(Nepal_seq_clusters_dict)[i]]]), col = "grey", density = 10, angle = 45)
  lines((names(Nepal_GPSC_numbers_per_good_year[["1"]])), prev_points + Nepal_GPSC_numbers_per_good_year[[unique(Nepal_seq_clusters_dict)[order(Nepal_VT_cols)[i]]]]/seq_no_per_year_filtered_goodYears, col = sort(Nepal_VT_cols)[i])
  points((names(Nepal_GPSC_numbers_per_good_year[["1"]])), prev_points + Nepal_GPSC_numbers_per_good_year[[unique(Nepal_seq_clusters_dict)[order(Nepal_VT_cols)[i]]]]/seq_no_per_year_filtered_goodYears, col = sort(Nepal_VT_cols)[i])
...
# create separate plots for VTs and NVTs
VT_Nepal_seq_clusters_dict_common <- Nepal_seq_clusters_dict_common[which(Nepal_VT == 1)]
NVT_Nepal_seq_clusters_dict_common <- Nepal_seq_clusters_dict_common[which(Nepal_VT == 0)]

plot((names(Nepal_GPSC_numbers_per_good_year[["1"]])), Nepal_GPSC_numbers_per_good_year[["1"]]/seq_no_per_year_filtered_goodYears, type = "l", ylim = c(0, 0.3))
for (i in which(Nepal_VT == 1)) {
  #lines((names(Nepal_GPSC_numbers_per_year_conf_low[["1"]])), Nepal_GPSC_numbers_per_year_conf_low[[unique(Nepal_seq_clusters_dict)[i]]], col = Nepal_VT_cols_2[i])
  #lines((names(Nepal_GPSC_numbers_per_year_conf_up[["1"]])), Nepal_GPSC_numbers_per_year_conf_up[[unique(Nepal_seq_clusters_dict)[i]]], col = Nepal_VT_cols_2[i])
  #polygon(c((names(Nepal_GPSC_numbers_per_year[["1"]])), (names(Nepal_GPSC_numbers_per_year[["1"]]))), c(Nepal_GPSC_numbers_per_year_conf_low[[unique(Nepal_seq_clusters_dict)[i]]], Nepal_GPSC_numbers_per_year_conf_up[[unique(Nepal_seq_clusters_dict)[i]]]), col = "grey", density = 10, angle = 45)
  lines((names(Nepal_GPSC_numbers_per_good_year[["1"]])), Nepal_GPSC_numbers_per_good_year[[unique(Nepal_seq_clusters_dict)[i]]]/seq_no_per_year_filtered_goodYears, col = Nepal_VT_cols[i])
  points((names(Nepal_GPSC_numbers_per_good_year[["1"]])), Nepal_GPSC_numbers_per_good_year[[unique(Nepal_seq_clusters_dict)[i]]]/seq_no_per_year_filtered_goodYears, col = Nepal_VT_cols[i])
}

# together, not stacked, NVTs in background
Nepal_VT_cols_v2 <- Nepal_VT_cols
Nepal_VT_cols_v2[which(Nepal_VT_cols_v2 == "black")] <- "#00000010"
plot((names(Nepal_GPSC_numbers_per_good_year[["1"]])), Nepal_GPSC_numbers_per_good_year[["1"]]/seq_no_per_year_filtered_goodYears, type = "l", ylim = c(0, 0.15), lwd = 2)
for (i in 1:length(Nepal_seq_clusters_dict_common <- Nepal_seq_clusters_dict_common)) {
  #lines((names(Nepal_GPSC_numbers_per_year_conf_low[["1"]])), Nepal_GPSC_numbers_per_year_conf_low[[unique(Nepal_seq_clusters_dict)[i]]], col = Nepal_VT_cols_2[i])
  #lines((names(Nepal_GPSC_numbers_per_year_conf_up[["1"]])), Nepal_GPSC_numbers_per_year_conf_up[[unique(Nepal_seq_clusters_dict)[i]]], col = Nepal_VT_cols_2[i])
  #polygon(c((names(Nepal_GPSC_numbers_per_year[["1"]])), (names(Nepal_GPSC_numbers_per_year[["1"]]))), c(Nepal_GPSC_numbers_per_year_conf_low[[unique(Nepal_seq_clusters_dict)[i]]], Nepal_GPSC_numbers_per_year_conf_up[[unique(Nepal_seq_clusters_dict)[i]]]), col = "grey", density = 10, angle = 45)
  lines((names(Nepal_GPSC_numbers_per_good_year[["1"]])), Nepal_GPSC_numbers_per_good_year[[unique(Nepal_seq_clusters_dict)[i]]]/seq_no_per_year_filtered_goodYears, col = Nepal_VT_cols_v2[i], lwd = 2)
  points((names(Nepal_GPSC_numbers_per_good_year[["1"]])), Nepal_GPSC_numbers_per_good_year[[unique(Nepal_seq_clusters_dict)[i]]]/seq_no_per_year_filtered_goodYears, col = Nepal_VT_cols_v2[i], lwd = 2)
}
```
...
# create df
time <- as.integer(rep(names(seq_no_per_year_filtered_goodYears),times=length(names(Nepal_GPSC_numbers_per_good_year))))  # x Axis
value <- unlist(Nepal_GPSC_numbers_per_good_year, use.names=FALSE) # y Axis
group <- rep(names(Nepal_GPSC_numbers_per_good_year),each=length(seq_no_per_year_filtered_goodYears))        # group, one shape per group
Nepal_VT_mixed_012 <- Nepal_VT_mixed
Nepal_VT_mixed_012[which(Nepal_VT_mixed == 1)] <- "VT"
Nepal_VT_mixed_012[which(Nepal_VT_mixed == 0)] <- "NVT"
Nepal_VT_mixed_012[intersect(which(Nepal_VT_mixed > 0), which(Nepal_VT_mixed <1))] <- "mixed"
vt <- as.character(rep(Nepal_VT_mixed_012, each=length(seq_no_per_year_filtered_goodYears)))
Nepal_VT_for_df <- Nepal_VT
Nepal_VT_for_df[which(Nepal_VT == 1)] <- "VT"
Nepal_VT_for_df[which(Nepal_VT == 0)] <- "NVT"
vt_binary <- as.character(rep(Nepal_VT_for_df, each=length(seq_no_per_year_filtered_goodYears)))
Nepal_gpscs_over_time_df <- data.frame(time, value, group,vt,vt_binary)
Nepal_gpscs_over_time_df_ord <- Nepal_gpscs_over_time_df[rev(order(vt_binary)),]
Nepal_gpscs_over_time_df_ord$group <- factor(Nepal_gpscs_over_time_df_ord$group , levels=unique(Nepal_PP$GPSC)[order(Nepal_VT, decreasing = FALSE)] )

grouped_df <-
  Nepal_gpscs_over_time_df_ord %>%
  group_by(vt_binary, time) %>%
  summarise(sum_val = sum(value))
grouped_df$vt_binary <- factor(grouped_df$vt_binary, levels= c("NVT","VT"))

# stacked area chart
ggplot(Nepal_gpscs_over_time_df_ord, aes(x=time, y=value, fill=group)) + 
    geom_area() + guides(fill="none") + 
  geom_vline(xintercept = 2015, size = 0.3) +
  annotate("text", x = 2015, y = 500, label = "Vaccine Introduction", angle=90,vjust = -1) + ggtitle("Counts of Vaccine Types and Non-Vaccine Types over time") +
  ylab("Count") + scale_x_continuous("Time", breaks=seq(2010,2020,1)) 
...
ggplot(Nepal_gpscs_over_time_df_ord, aes(x=time, y=value, fill=group)) + 
  ...
  annotate("text", x = 2015, y = 500, label = "Vaccine Introduction", angle=90,vjust = -1) + ggtitle("Counts of Vaccine Types and Non-Vaccine Types over time") +
  ylab("Count") + scale_x_continuous("Time", breaks=seq(2010,2020,1)) +
  geom_line(data=grouped_df, position = "stack", aes(x=time, y = sum_val, fill = vt_binary), linewidth = 1) 
 

# create df for plotting by vt 
Nepal_gpscs_over_time_df_vt <- Nepal_gpscs_over_time_df_ord %>% 
  group_by(vt, time) %>% 
  summarise(across(value, sum))

ggplot(Nepal_gpscs_over_time_df_vt, aes(x=time, y=value, fill=vt)) + 
    geom_area()

# proportional stacked area plot
Nepal_gpscs_over_time_df_rel <- Nepal_gpscs_over_time_df_ord  %>%
  group_by(time, group) %>%
  summarise(n = sum(value)) %>%
  mutate(percentage = n / sum(n))
Nepal_gpscs_over_time_df_rel$group <- factor(Nepal_gpscs_over_time_df_rel$group , levels=unique(Nepal_PP$GPSC)[order(Nepal_VT, decreasing = FALSE)] )

grouped_rel_df <-
  Nepal_gpscs_over_time_df_ord %>%
  group_by(time, vt_binary) %>%
  summarise(n = sum(value)) %>%
  mutate(percentage = n / sum(n))
  
grouped_rel_df$vt_binary <- factor(grouped_rel_df$vt_binary, levels= c("NVT","VT"))

# Plot
ggplot(Nepal_gpscs_over_time_df_rel, aes(x=time, y=percentage, fill=group)) + 
    geom_area(alpha=0.6 , size=0.1, colour="black", linetype = 3) + guides(fill="none") +
  geom_vline(xintercept = 2015, size = 0.3) +
  annotate("text", x = 2015, y = 0.2, label = "Vaccine Introduction", angle=90,vjust = -1) + ggtitle("Counts of Vaccine Types and Non-Vaccine Types over time") +
  ylab("Count") + scale_x_continuous("Time", breaks=seq(2010,2020,1)) +
  geom_line(data=grouped_rel_df, position = "stack", aes(x=time, y = percentage, fill = vt_binary), linewidth = 1) 



Nepal_gpscs_over_time_df_vt_rel <- Nepal_gpscs_over_time_df_vt  %>%
  group_by(time, vt) %>%
  summarise(n = sum(value)) %>%
  mutate(percentage = n / sum(n))

ggplot(Nepal_gpscs_over_time_df_vt_rel, aes(x=time, y=percentage, fill=vt)) + 
    geom_area(alpha=0.6 , size=1, colour="black") + 
  geom_vline(xintercept = 2015, size = 0.3) +
  annotate("text", x = 2015, y = 0.35, label = "Vaccine Introduction", angle=90,vjust = -1)

# try this with binary VT
Nepal_gpscs_over_time_df_binvt <- Nepal_gpscs_over_time_df %>% 
  group_by(vt_binary, time) %>% 
  summarise(across(value, sum))

ggplot(Nepal_gpscs_over_time_df_binvt, aes(x=time, y=value, fill=vt_binary)) + 
    geom_area() + 
  geom_vline(xintercept = 2015, size = 0.3) +
  annotate("text", x = 2015, y = 500, label = "Vaccine Introduction", angle=90,vjust = -1) + ggtitle("Counts of Vaccine Types and Non-Vaccine Types over time") +
  ylab("Count") + scale_x_continuous("Time", breaks=seq(2010,2020,1))
...
ggplot(Nepal_gpscs_over_time_df_binvt, aes(x=time, y=value, fill=vt_binary)) + 
    geom_area() + guides(colour="none") + 
  geom_vline(xintercept = 2015, size = 0.3) +
  annotate("text", x = 2015, y = 500, label = "Vaccine Introduction", angle=90,vjust = -1) + ggtitle("Counts of Vaccine Types and Non-Vaccine Types over time") +
  ylab("Count") + scale_x_continuous("Time", breaks=seq(2010,2020,1)) + 
  geom_line(data=Nepal_gpscs_over_time_df_ord, position = "stack", aes(x=time, y = value, colour = group), linetype = 3)  +
    scale_color_manual(values=rep('black',173))


Nepal_gpscs_over_time_df_binvt_rel <- Nepal_gpscs_over_time_df_binvt  %>%
  group_by(time, vt_binary) %>%
  summarise(n = sum(value)) %>%
  mutate(percentage = n / sum(n))

ggplot(Nepal_gpscs_over_time_df_binvt_rel, aes(x=time, y=percentage, fill=vt_binary)) + 
    geom_area(alpha=0.6 , size=1, colour="black") + 
  geom_vline(xintercept = 2015, size = 0.3) +
  annotate("text", x = 2015, y = 0.55, label = "Vaccine Introduction", angle=90,vjust = -1) + ggtitle("Proportion of Vaccine Types and Non-Vaccine Types over time") +
  ylab("Proportion") + scale_x_continuous("Time", breaks=seq(2010,2020,1))

ggplot(Nepal_gpscs_over_time_df_binvt_rel, aes(x=time, y=percentage, fill=vt_binary)) + guides(colour="none") +
geom_area(alpha=0.6 , size=1, colour="black") + ggtitle("Proportion of Vaccine Types and Non-Vaccine Types over time") +
  ylab("Proportion") + scale_x_continuous("Time", breaks=seq(2010,2020,1)) +
  geom_line(data=Nepal_gpscs_over_time_df_rel, position = "stack", aes(x=time, y = percentage, colour = group, fill = NULL), linetype = 3) +
    scale_color_manual(values=rep('#00000050',173)) + 
  geom_vline(xintercept = 2015, size = 0.5, colour = "red") +
  annotate("text", x = 2016, y = 0.95, colour = "red", label = "Vaccine Introduction", hjust = 0.75) 
```
