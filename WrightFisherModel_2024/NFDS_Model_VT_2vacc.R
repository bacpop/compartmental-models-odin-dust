...
Pop_size <- sum(Pop[1:species_no,1:2])
...
probs[,] <- ((1 + exp(sigma_f))^pi_f_genotypes[i] * (1 + exp(sigma_w))^pi_w_genotypes[i]) * Pop[i,j] * (1- (as.integer(time >= vacc_time1) * as.integer(time < vacc_time2) * vT1[j] * v)) * (1- (as.integer(time >= vacc_time2) * vT2[j] * v))
...
vT1[] <- user() # Boolean vector (0/1) whether genotype is affected by vaccine (1) or not (0)
vT2[] <- user() # Boolean vector (0/1) whether genotype is affected by vaccine (1) or not (0)
...
dim(freq) <- gene_no
dim(Pop_ini) <- c(species_no,3)
dim(Pop_eq) <- species_no
dim(Pop_mig) <- c(species_no,3)
...
dim(Pop) <- c(species_no,3)
dim(y) <- c(species_no,3)
dim(probs) <- c(species_no,3)
dim(vT1) <- 3
dim(vT2) <- 3
dim(migVec) <- c(species_no,3)
