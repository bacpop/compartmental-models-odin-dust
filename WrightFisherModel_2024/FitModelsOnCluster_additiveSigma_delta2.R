...
if(args[1] == "ggCaller" & args[2] == "PopPUNK"){
  ...
  delta_ranking <- readRDS(file = "ggC_delta_ranking_post-pre.rds")
  ...
  output_filename <- "ggCaller_PopPUNK_addSigma_delta2"
...
mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", -2, min =-1000, max = 0), mcstate::pmcmc_parameter("sigma_w", -10, min = -1000, max = 0), mcstate::pmcmc_parameter("prop_f", 0.25, min = 0, max = 1), mcstate::pmcmc_parameter("m", -4, min =-1000, max = 0), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 1)), proposal_matrix, make_transform(complex_params))
