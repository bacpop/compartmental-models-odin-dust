...
op <- par(mar = c(5,7,4,2) + 0.1,mgp=c(3,1,0))
plot(cluster_freq_1 / sum(cluster_freq_1), cluster_freq_3 / sum(cluster_freq_3), axes = FALSE, ann = FALSE, pch = 19, col = VT_colours)
...
peri_post <- (data_gene_freq_2 + data_gene_freq_3) / (nrow(subset(clusters,clusters$seqYear==2004)) + nrow(subset(clusters,clusters$seqYear==2007)))
...
expand_factor <- 15000 / sum(cluster_freq_1)
exp_noise <- 10
start_pop <- (sapply((cluster_freq_1 + rexp(n = length(cluster_freq_1), rate = exp_noise)) * expand_factor, rpois, n=1))
...
expand_factor <- 15000 / sum(cluster_freq_1)
exp_noise <- 10
model_start_pop <- as.double(sapply((cluster_freq_1 + rexp(n = length(cluster_freq_1), rate = exp_noise)) * expand_factor, rpois, n=1))

# visual check for the "sampling"
#plot(cluster_freq_1/sum(cluster_freq_1))
...
Corander_params <- list(dt = 1/36, species_no = no_clusters,  gene_no = nrow(cluster_gene_presence_absence), Pop_ini = model_start_pop, Pop_eq = model_start_pop, capacity = sum(model_start_pop), Genotypes = matrix_cluster_gene_presence_absence, sigma_f = 0.1363, sigma_w = 0.0023, prop_f = 0.2483, delta = delta_ranking, m = 0.0044, vaccTypes = isVT_vec, v = 0.0812, vacc_time = 1) 
...
adap_Corander_params <- list(dt = 1/36, species_no = no_clusters,  gene_no = nrow(cluster_gene_presence_absence), Pop_ini = model_start_pop, Pop_eq = model_start_pop, capacity = sum(model_start_pop), Genotypes = matrix_cluster_gene_presence_absence, sigma_f = 0.1363, sigma_w = 0.0023, prop_f = 0.2483, delta = delta_ranking, m = 0.0044, vaccTypes = isVT_vec, v = 0.1812, vacc_time = 1) 
params_n_vP <- adap_Corander_params
```
...
### testing this whether it helps with the fitting:
peripost_fitting_cluster_freq_df$year <- c(1,2)
...
#the original:
# dt <- 1/36
###maybe I do need the 2001 data for fitting? Because what do I do otherwise with the first model point?
#all_fitting_freq_df <- fitting_cluster_freq_df
#all_fitting_freq_df$year <- c(1,2,3)
#mass_data <- mcstate::particle_filter_data(data = all_fitting_freq_df,
#                                          time = "year",
#                                          rate = 1 / dt,
#                                          initial_time = 0)
```
...
combined_compare <- function(state, observed, pars = NULL) {
  ...
  for (i in 1:62){
    #print(observed[as.character(ind)])
    #print(state[1+i, , drop = TRUE])
    # just checking one particle
    #result <- result + ll_pois(observed[[as.character(i)]], state[1+i,1 , drop = TRUE])
    #used to be this:
    result <- result + ll_pois(observed[[as.character(i)]], state[1+i, , drop = TRUE]/model_size * data_size)
    #but I don't really want to look at all the particles, or do I?
  }
  result
}
...
# vaccine types seem to be less successful
#x_mean <- x
library(hrbrthemes)
x_mean <- apply(x, c(1,3), mean)
...
expand_factor <- 15000 / sum(cluster_freq_1)
exp_noise <- 10
model_start_pop <- (sapply((cluster_freq_1 + rexp(n = length(cluster_freq_1), rate = exp_noise)) * expand_factor, rpois, n=1))
...
mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", 0.15, min = 0.1, max = 0.5), mcstate::pmcmc_parameter("sigma_w", 0.005, min = 0, max = 0.0749), mcstate::pmcmc_parameter("prop_f", 0.25, min = 0, max = 1), mcstate::pmcmc_parameter("m", 0.03, min = 0, max = 0.1), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 1)), proposal_matrix, transform)
mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", 0.15, min = 0, max = 1), mcstate::pmcmc_parameter("sigma_w", 0.005, min = 0, max = 1), mcstate::pmcmc_parameter("prop_f", 0.25, min = 0, max = 1), mcstate::pmcmc_parameter("m", 0.03, min = 0, max = 0.1), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 1)), proposal_matrix, transform)
...
par(mfrow = c(1,1))
true_history <- peripost_fitting_cluster_freq_df
plot_particle_filter(det_pmcmc_run$trajectories$state/15000*300, true_history, 1:2)
```
...
# in det_pmcmc_run$trajectories$state[,i,j] for each j and all i the vectors are exactly the same! 
# Why?! where is the same in storing that information then? I though those are supposed to be the 
```
...
```{r}
stoch_proposal_matrix2 <- cov(pmcmc_run2$pars)
stoch_mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", 0.15, min = 0.1, max = 0.5), mcstate::pmcmc_parameter("sigma_w", 0.005, min = 0, max = 0.05), mcstate::pmcmc_parameter("prop_f", 0.25, min = 0, max = 1), mcstate::pmcmc_parameter("m", 0.03, min = 0, max = 0.1), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 0.5)), stoch_proposal_matrix, transform)
stoch_proposal_matrix2
```
...
control <- mcstate::pmcmc_control(
    ...
    progress = TRUE,
    n_chains = 1)
pmcmc_run3 <- mcstate::pmcmc(mcmc_pars, filter, control = control)
```

```{r}
stoch_mcmc3 <- coda::as.mcmc(cbind(pmcmc_run3$probabilities, pmcmc_run3$pars))

summary(stoch_mcmc3)
plot(stoch_mcmc3)
1 - coda::rejectionRate(stoch_mcmc3)

parameter_mean <- apply(pmcmc_run3$pars, 2, mean)
parameter_mean
```
...
lollipop_data_2001 <- data.frame(
  x=1:no_clusters, 
  model2001=pmcmc_run3$trajectories$state[2:63,1,1]/sum(pmcmc_run3$trajectories$state[2:63,1,1]),
  data2001=as.numeric(cluster_freq_1)/sum(as.numeric(cluster_freq_1))
)
lollipop_data_2004 <- data.frame(
  x=1:no_clusters, 
  model2004=pmcmc_run3$trajectories$state[2:63,1,2]/sum(pmcmc_run3$trajectories$state[2:63,1,2]),
  data2004=as.numeric(true_history[1,2:63])/sum(as.numeric(true_history[1,2:63]))
)
lollipop_data_2007 <- data.frame(
  x=1:no_clusters, 
  model2007=pmcmc_run3$trajectories$state[2:63,1,3]/sum(pmcmc_run3$trajectories$state[2:63,1,3]),
...
lollipop_plot_2007 <- ggplot(lollipop_data_2007) +
  ...
  ylab("Value of Y") + 
  ylim(0, max(max(lollipop_data_2004$model2004),max(lollipop_data_2004$data2004),max(lollipop_data_2007$model2007),max(lollipop_data_2007$data2007)))
grid.arrange(lollipop_plot_2001, lollipop_plot_2004, lollipop_plot_2007, ncol = 3)
...
```{r}
# function to run multiple determinstic fits in sequence
make_det_runs <- function(n, pre_fit){
  
  det_proposal_matrix <- cov(pre_fit$pars)
  det_mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", 0.15, min = 0, max = 1), mcstate::pmcmc_parameter("sigma_w", 0.05, min = 0, max = 1), mcstate::pmcmc_parameter("prop_f", 0.25, min = 0, max = 1), mcstate::pmcmc_parameter("m", 0.03, min = 0, max = 1), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 1)), det_proposal_matrix, transform)
  det_proposal_matrix
  det_fits <- c()
  ...
  for (i in 1:n){
    ...
    # runs in 4 minutes per chain right now
    detmcmc_curr <- coda::as.mcmc(cbind(
    det_pmcmc_tuned_run$probabilities, det_pmcmc_tuned_run$pars))
  
    det_fits <- c(det_fits, det_pmcmc_tuned_run)
    det_proposal_matrix <- cov(det_pmcmc_tuned_run$pars)
    det_mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", 0.15, min = 0, max = 1), mcstate::pmcmc_parameter("sigma_w", 0.05, min = 0, max = 1), mcstate::pmcmc_parameter("prop_f", 0.25, min = 0, max = 1), mcstate::pmcmc_parameter("m", 0.03, min = 0, max = 1), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 1)), det_proposal_matrix, transform)
    #det_proposal_matrix
  }
...
```{r}
coda::effectiveSize(detmcmc_curr)
...
sigmaf <- cbind(det_pmcmc_tuned_run$pars[,'sigma_f'][1:1000], det_pmcmc_tuned_run$pars[,'sigma_f'][1001:2000], det_pmcmc_tuned_run$pars[,'sigma_f'][2001:3000], det_pmcmc_tuned_run$pars[,'sigma_f'][3001:4000])
sigmaw <- cbind(det_pmcmc_tuned_run$pars[,'sigma_w'][1:1000], det_pmcmc_tuned_run$pars[,'sigma_w'][1001:2000], det_pmcmc_tuned_run$pars[,'sigma_w'][2001:3000], det_pmcmc_tuned_run$pars[,'sigma_w'][3001:4000])
propf <- cbind(det_pmcmc_tuned_run$pars[,'prop_f'][1:1000], det_pmcmc_tuned_run$pars[,'prop_f'][1001:2000], det_pmcmc_tuned_run$pars[,'prop_f'][2001:3000], det_pmcmc_tuned_run$pars[,'prop_f'][3001:4000])
mig <- cbind(det_pmcmc_tuned_run$pars[,'m'][1:1000], det_pmcmc_tuned_run$pars[,'m'][1001:2000], det_pmcmc_tuned_run$pars[,'m'][2001:3000], det_pmcmc_tuned_run$pars[,'m'][3001:4000])
vacc <- cbind(det_pmcmc_tuned_run$pars[,'v'][1:1000], det_pmcmc_tuned_run$pars[,'v'][1001:2000], det_pmcmc_tuned_run$pars[,'v'][2001:3000], det_pmcmc_tuned_run$pars[,'v'][3001:4000])
...
# result: the parameter space gets very well explored but probably no convergence?!
...
control <- mcstate::pmcmc_control(
    ...
    progress = TRUE,
    n_chains = 1)
det_pmcmc_tuned_run1 <- mcstate::pmcmc(det_mcmc_pars, det_filter, control = control)
det_pmcmc_tuned_run2 <- mcstate::pmcmc(det_mcmc_pars, det_filter, control = control)
det_pmcmc_tuned_run3 <- mcstate::pmcmc(det_mcmc_pars, det_filter, control = control)
det_pmcmc_tuned_run4 <- mcstate::pmcmc(det_mcmc_pars, det_filter, control = control)

coda_run1 <- coda::as.mcmc(cbind(det_pmcmc_tuned_run1$probabilities, det_pmcmc_tuned_run1$pars))
coda_run2 <- coda::as.mcmc(cbind(det_pmcmc_tuned_run2$probabilities, det_pmcmc_tuned_run2$pars))
coda_run3 <- coda::as.mcmc(cbind(det_pmcmc_tuned_run3$probabilities, det_pmcmc_tuned_run3$pars))
coda_run4 <- coda::as.mcmc(cbind(det_pmcmc_tuned_run4$probabilities, det_pmcmc_tuned_run4$pars))

coda_list <- coda::mcmc.list(coda_run1, coda_run2,coda_run3,coda_run3)

coda_run <- coda::as.mcmc(cbind(det_pmcmc_tuned_run$probabilities, det_pmcmc_tuned_run$pars))

# use coda to compute r hat
library(coda)
gelman.diag(coda_list, confidence = 0.95, transform=FALSE, autoburnin=TRUE,multivariate=FALSE)

# I had to set multivariate=FALSE otherwise I had an issue with the covariance matrix not being positive definite
# here probably because of the non-changing prior

# I read that you should only trust values that have an R hat of below 1.05 (or <=1.05) which is true here
# but probably you can see that m and v are less good than the other values. possibly not converged yet?
```


```{r}
#I also still have the issue that sigma_f and sigma_w do not always behave as sigma_f > sigma_w because the algorithm does not know that I want that.
# How can I solve that?
# I could, for each iteration, enforce sigma_w < former sigma_f value estimate?
# I am sure thre are better ways, but I can try that now
...
control <- mcstate::pmcmc_control(
    ...
    n_chains = 4)
det_pmcmc_run <- mcstate::pmcmc(mcmc_pars, det_filter, control = control)
#creating a sequence of deterministic runs

# function to run multiple determinstic fits in sequence
make_det_runs_restrictedSigmas <- function(n, pre_fit){
  
  det_proposal_matrix <- cov(pre_fit$pars)
  det_mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", 0.5, min = 0.5, max = 1), mcstate::pmcmc_parameter("sigma_w", 0.05, min = 0, max = 0.5), mcstate::pmcmc_parameter("prop_f", 0.25, min = 0, max = 1), mcstate::pmcmc_parameter("m", 0.03, min = 0, max = 1), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 1)), det_proposal_matrix, transform)
  det_proposal_matrix
  det_fits <- c()
  ...
  for (i in 1:n){
    ...
    det_proposal_matrix <- cov(det_pmcmc_tuned_run$pars)
    det_mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", max(0.15,mean(det_pmcmc_tuned_run$pars[,"sigma_w"])), min =  mean(det_pmcmc_tuned_run$pars[,"sigma_w"]), max = 1), mcstate::pmcmc_parameter("sigma_w", min(0.05,mean(det_pmcmc_tuned_run$pars[,"sigma_f"])), min = 0, max = mean(det_pmcmc_tuned_run$pars[,"sigma_f"])), mcstate::pmcmc_parameter("prop_f", 0.25, min = 0, max = 1), mcstate::pmcmc_parameter("m", 0.03, min = 0, max = 1), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 1)), det_proposal_matrix, transform)
    #det_proposal_matrix
  }
  return(detmcmc_curr)
}
...
det_runs <- make_det_runs_restrictedSigmas(2,det_pmcmc_run)

det_run2 <- coda::as.mcmc(cbind(det_pmcmc_tuned_run$probabilities, det_pmcmc_tuned_run$pars))
summary(det_run2)
plot(det_run2)
```
...
lollipop_data_2004 <- data.frame(
  x=1:no_clusters, 
  model2004=det_pmcmc_tuned_run$trajectories$state[2:63,1,2]/sum(det_pmcmc_tuned_run$trajectories$state[2:63,1,2]),
  data2004=as.numeric(true_history[1,2:63])/sum(as.numeric(true_history[1,2:63]))
)
lollipop_data_2007 <- data.frame(
  x=1:no_clusters, 
  model2007=det_pmcmc_tuned_run$trajectories$state[2:63,1,3]/sum(det_pmcmc_tuned_run$trajectories$state[2:63,1,3]),
  data2007=as.numeric(true_history[2,2:63])/sum(as.numeric(true_history[2,2:63]))
)
# Change baseline
lollipop_plot_2004 <- ggplot(lollipop_data_2004) +
