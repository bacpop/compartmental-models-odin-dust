...
Genotypes <- as.matrix(mean_mass_cluster_freq[,-1])

#capacity <- sum(cluster_freq_1)
capacity <- sum(model_start_pop)
delta <- delta_ranking
vaccTypes <- mass_VT_mean
vacc_time <- 0
dt <- 1/36
...
make_lollipop_plot(data1 = mass_cluster_freq_1, data2 = mass_cluster_freq_2, data3 = mass_cluster_freq_3, model1 = det_pmcmc_run$trajectories$state[-1,1,1], model2 = det_pmcmc_run$trajectories$state[-1,1,2], model3 = det_pmcmc_run$trajectories$state[-1,1,3])
...
```{r}
#use lollipop plotting function
make_lollipop_plot(data1 = mass_cluster_freq_1, data2 = mass_cluster_freq_2, data3 = mass_cluster_freq_3, model1 = det_pmcmc_run2$trajectories$state[-1,1,1], model2 = det_pmcmc_run2$trajectories$state[-1,1,2], model3 = det_pmcmc_run2$trajectories$state[-1,1,3])
```
...
```{r}
stoch_proposal_matrix <- cov(det_pmcmc_run2$pars)
stoch_mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", 0.15, min = 0.075, max = 0.5), mcstate::pmcmc_parameter("sigma_w", 0.005, min = 0, max = 0.05), mcstate::pmcmc_parameter("prop_f", 0.25, min = 0, max = 1), mcstate::pmcmc_parameter("m", 0.03, min = 0, max = 0.1), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 1)), det_proposal_matrix, make_transform(complex_params))
...
stoch_mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", 0.15, min = 0.075, max = 0.5), mcstate::pmcmc_parameter("sigma_w", 0.005, min = 0, max = 0.05), mcstate::pmcmc_parameter("prop_f", 0.25, min = 0, max = 1), mcstate::pmcmc_parameter("m", 0.03, min = 0, max = 0.1), mcstate::pmcmc_parameter("v", 0.05, min = 0, max = 0.5)), stoch_proposal_matrix, make_transform(complex_params))
stoch_proposal_matrix
```
...
```{r}
processed_chains <- mcstate::pmcmc_thin(pmcmc_run2, burnin = 50, thin = 1)
parameter_mean_hpd <- apply(processed_chains$pars, 2, mean)
parameter_mean_hpd
```
...
```{r}
#use lollipop plotting function
make_lollipop_plot(data1 = mass_cluster_freq_1, data2 = mass_cluster_freq_2, data3 = mass_cluster_freq_3, model1 = pmcmc_run2$trajectories$state[-1,1,1], model2 = pmcmc_run2$trajectories$state[-1,1,2], model3 = pmcmc_run2$trajectories$state[-1,1,3])

```

#stoch Fit 3
```{r}
stoch_proposal_matrix <- cov(pmcmc_run2$pars)
stoch_mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", parameter_mean_hpd[1], min = 0.075, max = 0.5), mcstate::pmcmc_parameter("sigma_w", parameter_mean_hpd[2], min = 0, max = 0.05), mcstate::pmcmc_parameter("prop_f", parameter_mean_hpd[3], min = 0, max = 1), mcstate::pmcmc_parameter("m", parameter_mean_hpd[4], min = 0, max = 0.1), mcstate::pmcmc_parameter("v", parameter_mean_hpd[5], min = 0, max = 0.5)), stoch_proposal_matrix, make_transform(complex_params))
stoch_proposal_matrix
```
...
control <- mcstate::pmcmc_control(
    ...
    progress = TRUE,
    n_chains = 1)
pmcmc_run3 <- mcstate::pmcmc(stoch_mcmc_pars, filter, control = control)
...
```{r}
processed_chains <- mcstate::pmcmc_thin(pmcmc_run3, burnin = 50, thin = 1)
parameter_mean_hpd <- apply(processed_chains$pars, 2, mean)
parameter_mean_hpd
```
...
```{r}
#use lollipop plotting function
make_lollipop_plot(data1 = mass_cluster_freq_1, data2 = mass_cluster_freq_2, data3 = mass_cluster_freq_3, model1 = pmcmc_run3$trajectories$state[-1,1,1], model2 = pmcmc_run3$trajectories$state[-1,1,2], model3 = pmcmc_run3$trajectories$state[-1,1,3])
...
```{r}
stoch_proposal_matrix <- cov(pmcmc_run3$pars)
stoch_mcmc_pars <- mcstate::pmcmc_parameters$new(list(mcstate::pmcmc_parameter("sigma_f", parameter_mean_hpd[1], min = 0.075, max = 0.5), mcstate::pmcmc_parameter("sigma_w", parameter_mean_hpd[2], min = 0, max = 0.05), mcstate::pmcmc_parameter("prop_f", parameter_mean_hpd[3], min = 0, max = 1), mcstate::pmcmc_parameter("m", parameter_mean_hpd[4], min = 0, max = 0.1), mcstate::pmcmc_parameter("v", parameter_mean_hpd[5], min = 0, max = 0.5)), stoch_proposal_matrix, make_transform(complex_params))
stoch_proposal_matrix
```
...
control <- mcstate::pmcmc_control(
    ...
    progress = TRUE,
    n_chains = 2)
pmcmc_run4 <- mcstate::pmcmc(mcmc_pars, filter, control = control)

#par(mfrow = c(1,1))
#plot_particle_filter(pmcmc_run$trajectories$state, true_history, 1:2)

stoch_mcmc4 <- coda::as.mcmc(cbind(pmcmc_run4$probabilities, pmcmc_run4$pars))
plot(stoch_mcmc4)

1 - coda::rejectionRate(stoch_mcmc4)
```
