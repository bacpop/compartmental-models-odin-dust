...
# New idea: use variation to compute nfds directly (a/variation = nfds strength)
a_freq[,] <- Genotypes[i,j] * (eq[i] - freq[i]) * (a/delta2[i])
a_genotypes[] <- sum(a_freq[1:gene_no,i])
...
probs[] <- (1 + a_genotypes[i]) * Pop[i] * (1- (as.integer(time >= vacc_time) * vaccTypes[i] * v))
...
dim(eq) <- gene_no
dim(a_freq) <- c(gene_no, species_no)
dim(a_genotypes) <- species_no
