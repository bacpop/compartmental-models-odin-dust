...
r_output(readLines(file.path("WrightFisher_1Allele_diploid.R")))
```
...
```{r}
opts <- odin_dust_options(verbose = TRUE, workdir = "/Users/llorenz/Documents/PhD_Project/Code/dust_folder2", skip_cache = TRUE)
WF_1A_d <- odin.dust::odin_dust("WrightFisher_1Allele_diploid.R", options = opts)
```

### Define model parameters
```{r}
params <- list(dt = 1, Pop_ini = c(100,100,100), pop_size = 300) 
```


### Running the model:
```{r}
WFmodel_1A_d <- WF_1A_d$new(pars = params,
                         time = 1,
                         n_particles = 10L,
                         n_threads = 4L,
                         seed = 1L)
...
n_particles <- 10L
p_new <- params
WFmodel_1A_d$update_state(pars = p_new, time = 0)
WFmodel_1A_d$state()
```

```{r fig.height=5, fig.width=7}
n_times <- 500
x <- array(NA, dim = c(WFmodel_1A_d$info()$len, n_particles, n_times))

for (t in seq_len(n_times)) {
  x[ , , t] <- WFmodel_1A_d$run(t)
}
time <- x[1, 1, ]
x <- x[-1, , ]
