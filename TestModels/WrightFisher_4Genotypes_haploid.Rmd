...
r_output(readLines(file.path("WrightFisher_4Genotypes_haploid.R")))
```
...
```{r}
WF_4G_h <- odin.dust::odin_dust("WrightFisher_4Genotypes_haploid.R")
```

### Define model parameters
```{r}
params <- list(dt = 1, A_ini = .25, B_ini = .25, C_ini = .25, D_ini = .25, pop_size = 100) 
```


### Running the model:
```{r}
WFmodel_4G_h <- WF_4G_h$new(pars = params,
                         time = 1,
                         n_particles = 10L,
                         n_threads = 4L,
                         seed = 1L)
...
WFmodel_4G_h$run(30)
WFmodel_4G_h$run(40)
WFmodel_4G_h$run(132)
WFmodel_4G_h$run(133)
WFmodel_4G_h$run(134)
WFmodel_4G_h$run(135)
```
...
n_particles <- 10L
p_new <- params
WFmodel_4G_h$update_state(pars = p_new, time = 0)
WFmodel_4G_h$state()
```

```{r fig.height=5, fig.width=7}
n_times <- 500
x <- array(NA, dim = c(WFmodel_4G_h$info()$len, n_particles, n_times))

for (t in seq_len(n_times)) {
  x[ , , t] <- WFmodel_4G_h$run(t)
}
time <- x[1, 1, ]
x <- x[-1, , ]
