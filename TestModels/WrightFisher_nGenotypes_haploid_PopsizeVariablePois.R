...
y[] <- if (probs[1]/sum(probs[1:species_no]) < 1)
  rpois((probs[i]/sum(probs[1:species_no]))*Pop_size) else Pop_size 
...
#### DRAMATIC changes in Population size. Is that what we want??
# I definitively need a capacity term here then, I think.
...
#pop_size <- sum(Pop_ini[1:species_no])
