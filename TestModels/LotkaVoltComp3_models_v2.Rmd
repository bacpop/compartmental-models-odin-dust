...
r_output(readLines(file.path("LotkaVolterraComp3_v2.R")))
```
...
```{r}
library(odin.dust)
LotVolComp3v2 <- odin.dust::odin_dust("LotkaVolterraComp3_v2.R")
```

### Running the model:

```{r}
LVC_model3v2 <- LotVolComp3v2$new(pars = list(dt = 1,
                                     C_ini = c(200,200,200),
                                     r_C = c(0.2,0.2,0.2),
                                     K = c(500,500,500),
...
dt <- 1
n_particles <- 10L
p_new <- list(dt = 1, C_ini = c(200,200,200), r_C = c(0.2,0.2,0.2), K = c(200,200,200), alpha = matrix(data = c(1,.15,.1,.1,1,.12,.1,.11,1),nrow = 3, ncol = 3, byrow = TRUE, dimnames = list(c("R","C","C2"),c("R","C","C2"))))
