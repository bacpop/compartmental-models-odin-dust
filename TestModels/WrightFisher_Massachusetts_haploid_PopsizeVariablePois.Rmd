...
par(mfrow=c(1,3))
plot(data_gene_freq_1, data_gene_freq_2)
plot(data_gene_freq_1, data_gene_freq_3)
plot(data_gene_freq_2, data_gene_freq_3)
```
...
```{r}
params_n_vP <- list(dt = 1, species_no = no_clusters,  gene_no = nrow(cluster_gene_presence_absence), Pop_ini = cluster_freq_1, Pop_eq = cluster_freq_3, capacity = 1000, Genotypes = matrix_cluster_gene_presence_absence, sigma = 0.02, m = 0.00, vaccTypes = rep(0, no_clusters), v = 0, vacc_time = 100) 
```
...
```{r}
# it would be cool to have a plot that shows the gene frequencies :-)

# matrix_cluster_gene_presence_absence
# the rows are the genes, the columns the clusters

# x[,1,t] should be all cluster frequencies at time t for particle 1
...
for (species in 2:length(y[,1,1])) {
  matlines(time, (y[species,1 , ]), col = cols[species], lty = 1)
}
