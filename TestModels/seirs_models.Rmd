...
r_output(readLines(file.path("seirs.R")))
```
...
```{r}
library(odin.dust)
gen_seirs <- odin.dust::odin_dust("seirs.R")
```
...
```{r}
seirs_model <- gen_seir$new(pars = list(dt = 1,
                                     S_ini = 1000,
                                     E_ini = 5,
                                     I_ini = 10,
                                     R_ini = 20,
...
```{r}
seirs_model$state() # it does not work to set R_ini to 20 at the beginning. Why??? check that out. I want to see periodic behavior!
```
...
dt <- 0.25
n_particles <- 10L
p_new <- list(dt = dt, S_ini = 50, E_ini = 0, I_ini = 0, R_ini = 50, beta = 0, delta = 0, gamma = 0, omega = 0.9)
seirs_model$update_state(pars = p_new, time = 0)
seirs_model$state()
```
...
n_times <- 2000
x <- array(NA, dim = c(seirs_model$info()$len, n_particles, n_times))

for (t in seq_len(n_times)) {
  x[ , , t] <- seirs_model$run(t)
}
time <- x[1, 1, ]
x <- x[-1, , ]
